...
class HotSpring {
    ...
    HotSpring(List<String> damagedConditionRecordsOfWhichSpringsAreDamaged) {
        this.damagedConditionRecordsOfWhichSpringsAreDamaged = damagedConditionRecordsOfWhichSpringsAreDamaged;
        map = new HashMap<>();
    }

    long repairDamagedConditionRecordsOfWhichSpringsAreDamaged() {
     ...
     */
    long countAllPossibleArrangements(String springRecord, List<Long> groups) {
        ...
        if (firstChar == OPERATIONAL_SPRING) {
            ...
            long nrDamaged = groups.getFirst();
